...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `CLI-RESPONSE-PASSTHROUGH` | CLI 后端响应透传 | 已发布 1.0.4 仍使用统一 envelope；当前工作区已实现直接输出后端 HTTP body 并通过本地 V3，目标发布版本为 1.0.5 | `packages/shared`、业务查询命令与公共输出文档 | `20260827-[RELEASE]发布1.0.5维护版本` | 待生效 | [`./topics/CLI后端响应透传.md`](./topics/CLI后端响应透传.md) | 2026-08-27 / `aef3504` + 1.0.5 工作区（201 tests / 14 workspace builds） |
