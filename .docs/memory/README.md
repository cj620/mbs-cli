...
| `UNIFIED-SEMANTIC-DISCOVERY` | 统一语义发现 | 用户只执行自然语言 find；正式 find/describe 经 `/cli/cli-service/cli/api/recall...` 请求，CLI 消费 workflow/api/table 混合候选 | `packages/cli`、`packages/doris`、Skill | `20260812-[BUG]修复语义召回服务路由` | 1.0.1 本地已验证/待发布 | [`./topics/统一语义发现.md`](./topics/统一语义发现.md) | 2026-08-12 / 当前工作区 |
...
| `RELEASE-LINE-1X` | 1.0.0 长期维护分支 | `origin/1.0.0` 独立于 master；npm `maintenance-1=1.0.0`、`latest=0.1.58`，1.0.1 路由修复本地已验证待发布 | 全仓库、`packages/cli`、CI | `20260812-[BUG]修复语义召回服务路由` | 1.0.0 已发布 / 1.0.1 待发布 | [`./topics/1.0.0长期维护分支.md`](./topics/1.0.0长期维护分支.md) | 2026-08-12 / 当前工作区 |

## 当前技术债

- npm `maintenance-1` 尚未包含当前工作区的 `1.0.1` cli-service 路由修复；目标环境 embedding/Milvus、workflow 重建与 30 条 eval 尚未验收。
