...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `AUTH-CREDENTIAL-BOUNDARY` | MBS 认证凭据边界 | `MBS_KEY` 永不接触；当前工作区默认允许合法远程 HTTP 用于密码、长期 Token 和 refresh，不要求确认或 Origin 授权；npm 1.0.6 仍仅允许 HTTPS/loopback | CLI 登录、刷新、认证缓存、业务一次性重试、Docker 报表服务 | `20260904-[SECURITY]临时支持远程HTTP认证` / DEC-003 / DEC-004 / DEC-005 | 当前工作区本地 V3 通过，待发布与真实 HTTP 联调；npm 1.0.6 不含该能力 | [`./topics/MBS认证凭据边界.md`](./topics/MBS认证凭据边界.md) | 2026-09-04 / 工作区 `ba436b3` 基线 |
...
| `RELEASE-LINE-1X` | 1.0.0 长期维护分支 | `origin/1.0.0` 独立于 master；npm `maintenance-1=1.0.6` 已发布且 `latest` 保持 `0.1.59` | 全仓库、`packages/cli`、CI | `20260903-[RELEASE]发布1.0.6维护版本` | 已发布并完成官方源核验 | [`./topics/1.0.0长期维护分支.md`](./topics/1.0.0长期维护分支.md) | 2026-09-04 / `04da103` / `maintenance-1-v1.0.6` |
...
- 当前工作区已改为默认允许远程 HTTP 认证并完成本地 V3，但尚未提交、发布或使用真实目标 HTTP 凭据链路联调；明文传输风险由部署方承担，服务端 HTTPS 可用后应立即重新配置。
