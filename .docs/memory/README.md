...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `AUTH-CREDENTIAL-BOUNDARY` | MBS 认证凭据边界 | `MBS_KEY` 永不接触；CLI 互斥缓存登录型 `AUTH_REFRESH` 或手工管理型 `LongToken`，统一交换内存 Access Token 与兼容 `SESSION`；重新登录前先清空旧认证缓存 | CLI 登录、刷新、认证缓存、业务一次性重试、Docker 报表服务 | `20260903-[RELEASE]发布1.0.6维护版本` / DEC-003 / DEC-004 | 已随 npm 1.0.6 发布并完成官方源核验 / 待真实网关联调 | [`./topics/MBS认证凭据边界.md`](./topics/MBS认证凭据边界.md) | 2026-09-04 / `04da103` / npm 1.0.6 |
