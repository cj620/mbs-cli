...
- 状态：已随 npm `1.0.7` 发布并完成官方包核验，待真实环境联调
- 日期：2026-09-04
...
- npm `1.0.6` 仍执行严格 HTTPS/loopback 门禁；npm `1.0.7` 已包含本决策的默认 HTTP 能力。
...
- 2026-09-04：DEC-007 将远程 HTTP 临时兼容扩展到扫码登录。认证中心在 HTTP 回调中不创建或发送长期 Refresh；CLI 只在完全没有 Refresh 且唯一 SESSION 通过 current-user 验证后保存最长两小时上下文并警告不可刷新。HTTPS 双 Cookie、URL 校验和本决策的明文风险结论不变。该变更已本地验证、待发布。
