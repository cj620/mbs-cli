...
- 状态：部分被 DEC-008 替代；管理型 LongToken 扩展已随 npm `1.0.6` 发布并完成官方源命令核验，待真实网关联调
- 日期：2026-09-03
...
- 2026-09-08：DEC-008 部分替代第 4 点与第 5 点。受保护的当前用户认证缓存允许保存严格校验且未过期的 Access Token，并规定任何认证、交换、网络、响应或落盘失败都不得主动清空登录材料；只有显式 logout 或开始新 login 可以清理。长期凭据互斥、仅发认证中心、SESSION-only 兼容和 `MBS_KEY` 禁令保持有效。该修正已随 npm `maintenance-1=1.1.4` 发布，真实认证联调仍未执行。
