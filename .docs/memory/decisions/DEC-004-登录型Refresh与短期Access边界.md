...
- 2026-09-08：DEC-008 部分替代第 4 点与第 5 点。当前工作区允许在受保护的当前用户认证缓存中保存严格校验且未过期的 Access Token，并规定任何认证、交换、网络、响应或落盘失败都不得主动清空登录材料；只有显式 logout 或开始新 login 可以清理。长期凭据互斥、仅发认证中心、SESSION-only 兼容和 `MBS_KEY` 禁令保持有效。该修正已完成本地 V3，尚未发布或真实联调。
