...
- **当前状态**：普通业务命令及 `find`、`describe`、`serve` 远端路由扩展已随 npm `1.1.4` 发布；业务 500 认证误判修复已本地完成并计划随 1.1.5 发布
- **最后核验时间**：2026-09-08
...
- `20260908-[BUG]修复业务500认证误判并定位SESSION丢失` 已在本地移除业务 `code=500` 的认证映射：500 保持普通 `MBSError`、原 body 和退出码 1，不交换凭据或重试；计划随 npm maintenance-1 1.1.5 发布。
...
| 业务 500 历史认证映射 | 修正 | `20260908-[BUG]修复业务500认证误判并定位SESSION丢失` | shared HTTP 错误分类、业务重试与退出码 | npm maintenance-1 1.1.5 | 本地 V3 完成，待发布 |
