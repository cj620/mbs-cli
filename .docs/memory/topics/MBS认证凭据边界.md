...
- `mbs login --password` 在校验 HTTPS 后，通过终端隐藏输入直接调用认证中心；不启动浏览器，也不提供参数或环境变量凭据入口。
- 后台长期 Refresh Token 模式同样先校验 HTTPS，再通过终端隐藏输入；按 `Authorization: LongToken <token>` 调用兼容交换，保存不轮换 Token 与兼容 `SESSION`，不提供参数或环境变量入口。
