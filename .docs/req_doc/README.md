...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260904-[RELEASE]发布1.1.2维护版本` | RELEASE / SECURITY | 发布 compat-session 两类 Refresh 移除 `client-type: cli` 的修复；仅更新 npm `maintenance-1`，保持 `latest` 不变 | 🚀 执行中：功能提交与首次分支 CI 成功，正在升级并提交 1.1.2，待第二次 CI、标签、Release 与官方包核验 | Codex | [`./20260904-[RELEASE]发布1.1.2维护版本/`](./20260904-[RELEASE]发布1.1.2维护版本/) |
...
| `20260904-[BUG]刷新交换移除客户端类型头` | BUG / SECURITY | compat-session 不按 login/refresh 区分客户端分类头；移除 Cookie 与 LongToken 两类 Refresh 中的 `client-type: cli`，普通业务请求头不变 | ✅ 已完成 / 待发布：定向 17 项、shared 114 项、全仓 259 项和 14 包构建通过；真实认证、CI 与发布未执行 | 未发布 | [`./20260904-[BUG]刷新交换移除客户端类型头/`](./20260904-[BUG]刷新交换移除客户端类型头/) |
