...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260825-[RELEASE]发布1.0.4维护版本` | RELEASE | 发布接口请求体编码端到端支持；仅更新 npm `maintenance-1`，保持普通 `latest` 不变 | 🚧 执行中：版本升级与本地 V3，后续按分支 CI → 维护标签 → Release → 官方源核验执行 | Codex | [`./20260825-[RELEASE]发布1.0.4维护版本/`](./20260825-[RELEASE]发布1.0.4维护版本/) |
| `20260825-[FEATURE]接口请求体编码端到端支持` | FEATURE | `mbs request --api-id` 按后端权威详情编码 JSON、urlencoded、multipart、文本、XML 和二进制；manifest 生成与 serve 复用统一编码器 | ✅ 本地实现与 V3 完成：构建、全量测试、命令帮助及 manifest dry-run 通过；真实业务请求、目标网关和发布未执行 | Codex | [`./20260825-[FEATURE]接口请求体编码端到端支持/`](./20260825-[FEATURE]接口请求体编码端到端支持/) |
