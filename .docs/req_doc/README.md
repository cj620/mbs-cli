...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260918-[RELEASE]发布1.1.6维护版本` | RELEASE / L3 | 发布参数依赖自动解析、明确数据库目标直达查询、可恢复批量查询与流式 Excel 导出；仅更新 npm `maintenance-1` | 🚧 执行中：发布前基线、标签与 npm 槽位已核对，正在执行 V3 门禁 | Codex | [`./20260918-[RELEASE]发布1.1.6维护版本/`](./20260918-[RELEASE]发布1.1.6维护版本/) |
| `20260918-[FEATURE]增强批量查询与可恢复导出` | FEATURE / L3 | SQL 方言引用与键集游标分页；长时间导出执行前创建日志/断点，成功批次持久化，支持显式恢复或重新全量执行，Excel 流式生成，所有重试有界 | 🧪 待验收 / 待发布：17 项 export 测试、Skill 契约、生成 dry-run、323 项全仓测试、14 包构建、CLI help 和差异检查通过；未安装或执行真实大批量链路 | Codex | [`./20260918-[FEATURE]增强批量查询与可恢复导出/`](./20260918-[FEATURE]增强批量查询与可恢复导出/) |
| `20260918-[FEATURE]明确数据库目标直达查询` | FEATURE / L2 | 完整只读 SQL 或明确物理表跳过 `mbs find`，经 `my-tables` 权限与数据源确认后直达数据库链路；模糊业务目标仍使用统一语义发现 | 🧪 待验收 / 待发布：契约测试、生成 dry-run、308 项测试、14 包构建和差异检查通过；未安装或执行真实 MBS 查询 | Codex | [`./20260918-[FEATURE]明确数据库目标直达查询/`](./20260918-[FEATURE]明确数据库目标直达查询/) |
| `20260909-[FEATURE]自动解析接口参数依赖` | FEATURE / L3 | Agent 在最多 10 次 `mbs find` 内递归发现参数来源 API、执行只读查询并回填唯一结果；find 或参数值歧义、预算耗尽与环路时暂停等待用户决策 | 🧪 本地已验证 / 待发布与真实链路验收：契约测试、生成 dry-run、307 项测试、14 包构建、CLI Skill 冒烟和差异检查通过 | Codex | [`./20260909-[FEATURE]自动解析接口参数依赖/`](./20260909-[FEATURE]自动解析接口参数依赖/) |
