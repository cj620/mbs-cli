...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260827-[RELEASE]发布1.0.5维护版本` | RELEASE | 发布后端响应原样透传；仅更新 npm `maintenance-1`，保持普通 `latest` 不变 | 🚀 执行中 / 未交付：1.0.5 与标签未占用，版本和审计已建立，正在执行本地 V3 | Codex | [`./20260827-[RELEASE]发布1.0.5维护版本/`](./20260827-[RELEASE]发布1.0.5维护版本/) |
| `20260827-[FEATURE]透传后端响应内容` | FEATURE / BREAKING CONTRACT | 业务查询命令直接输出后端 HTTP body；业务与 HTTP 错误保留实际 body，认证刷新和非零退出码不变 | 🚀 发布执行中 / 未交付：201 条全量测试、14 个 workspace 构建和文档同步通过；目标版本 1.0.5，等待发布门禁 | Codex | [`./20260827-[FEATURE]透传后端响应内容/`](./20260827-[FEATURE]透传后端响应内容/) |
