...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260904-[RELEASE]发布1.1.1维护版本` | RELEASE / SECURITY | 提交 current-user 请求头修复并发布 npm `maintenance-1=1.1.1`；保持 `latest` 不变 | 🚧 执行中：功能提交与分支 CI 成功，正在提交并验证 1.1.1 版本 | Codex（当前执行者） | [`./20260904-[RELEASE]发布1.1.1维护版本/`](./20260904-[RELEASE]发布1.1.1维护版本/) |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260904-[BUG]当前用户查询移除客户端类型头` | BUG / SECURITY | `/auth/user/current` 只携带标准化 SESSION Cookie，不发送 `client-type: cli`；两类登录后 Refresh 契约保持 | ✅ 本地 V3 完成 / 待发布：定向 17 项、shared 114 项、全仓 259 项及 14 包构建通过；真实认证联调未执行 | 未发布 | [`./20260904-[BUG]当前用户查询移除客户端类型头/`](./20260904-[BUG]当前用户查询移除客户端类型头/) |
