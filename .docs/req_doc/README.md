...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260918-[FEATURE]增强批量查询与可恢复导出` | FEATURE / L3 | SQL 方言引用与键集游标分页；长时间导出执行前创建日志/断点，成功批次持久化，支持显式恢复或重新全量执行，Excel 流式生成，所有重试有界 | 📦 已随 npm `1.1.6` 发布 / 待真实链路验收：官方包版本、游标/恢复帮助和批量任务 Skill 通过；未执行真实大批量导出 | Codex | [`./20260918-[FEATURE]增强批量查询与可恢复导出/`](./20260918-[FEATURE]增强批量查询与可恢复导出/) |
| `20260918-[FEATURE]明确数据库目标直达查询` | FEATURE / L2 | 完整只读 SQL 或明确物理表跳过 `mbs find`，经 `my-tables` 权限与数据源确认后直达数据库链路；模糊业务目标仍使用统一语义发现 | 📦 已随 npm `1.1.6` 发布 / 待真实链路验收：官方包数据库 Skill 通过；未执行真实 `my-tables → query` | Codex | [`./20260918-[FEATURE]明确数据库目标直达查询/`](./20260918-[FEATURE]明确数据库目标直达查询/) |
| `20260909-[FEATURE]自动解析接口参数依赖` | FEATURE / L3 | Agent 在最多 10 次 `mbs find` 内递归发现参数来源 API、执行只读查询并回填唯一结果；find 或参数值歧义、预算耗尽与环路时暂停等待用户决策 | 📦 已随 npm `1.1.6` 发布 / 待真实链路验收：官方包参数依赖 Skill 通过；未执行真实店铺到销售查询链路 | Codex | [`./20260909-[FEATURE]自动解析接口参数依赖/`](./20260909-[FEATURE]自动解析接口参数依赖/) |
| `20260908-[BUG]修复业务500认证误判并定位SESSION丢失` | BUG / SECURITY | 修复业务 500 误触发认证刷新和 ERP 空用户 NPE；确认 auth-center 正常复用/轮换 SESSION，远程 ERP 运行态仍待逐跳核对 | 🧪 部分交付 / 待目标验收：CLI 已随 npm `1.1.5` 发布；ERP 与网关修复仍仅本地完成，服务端发布和目标运行态验收未执行 | Codex | [`./20260908-[BUG]修复业务500认证误判并定位SESSION丢失/`](./20260908-[BUG]修复业务500认证误判并定位SESSION丢失/) |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260921-[RELEASE]发布1.1.7维护版本` | RELEASE / L3 | 发布流式非 2xx 响应安全物化修复，并将维护与默认 npm 通道升级到 1.1.7 | ✅ 已完成 / 已发布：327 条测试、14 包构建、分支 CI、Release、两个 dist-tag、精确/默认官方安装及合成错误回归通过；本机活动 CLI 已升级 | 2026-09-21 | [`./20260921-[RELEASE]发布1.1.7维护版本/`](./20260921-[RELEASE]发布1.1.7维护版本/) |
| `20260918-[BUG]修复流式HTTP错误响应序列化` | BUG / L3 | `postStream` 的非 2xx body 是含 Socket 的 `IncomingMessage`；共享 HTTP seam 有界读取并恢复 JSON/文本后再分类，保持一次认证刷新与成功 NDJSON 直通 | ✅ 已完成 / 已随 npm `1.1.7` 发布：本地 V3、分支 CI、Release、官方包和本机活动安装回归通过；真实后端链路待复验 | 2026-09-21 | [`./20260918-[BUG]修复流式HTTP错误响应序列化/`](./20260918-[BUG]修复流式HTTP错误响应序列化/) |
