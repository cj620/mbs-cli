...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `20260908-[RELEASE]发布1.1.5维护版本` | RELEASE / SECURITY | 发布业务 500 不再误触发认证刷新；仅更新 npm `maintenance-1`，保持 `latest` 不变 | 🚧 执行中：本地 V3 已通过，正在执行功能/版本提交、CI、标签与 npm 核验 | Codex | [`./20260908-[RELEASE]发布1.1.5维护版本/`](./20260908-[RELEASE]发布1.1.5维护版本/) |
| `20260908-[BUG]修复业务500认证误判并定位SESSION丢失` | BUG / SECURITY | 修复业务 500 误触发认证刷新和 ERP 空用户 NPE；确认 auth-center 正常复用/轮换 SESSION，远程 ERP 运行态仍待逐跳核对 | 🧪 本地 V3 完成 / 待发布与目标验收：CLI 306 项、ERP 相关 8 项、网关 23 项及构建/打包通过 | Codex | [`./20260908-[BUG]修复业务500认证误判并定位SESSION丢失/`](./20260908-[BUG]修复业务500认证误判并定位SESSION丢失/) |
