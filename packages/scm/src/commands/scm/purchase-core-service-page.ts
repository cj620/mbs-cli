...
export default class ScmPurchaseCoreServicePage extends MBSCommand {
  ...
  async run(): Promise<void> {
    ...
    const data = await this.client.post('/purchase-core-service/down/cost/report/page', { "downCostStartDate": flags.downCostStartDate, "downCostEndDate": flags.downCostEndDate, "stockStartDate": flags.stockStartDate, "stockEndDate": flags.stockEndDate, "spu": flags.spu, "sku": flags.sku, "downCostPerson": flags.downCostPerson, "orderBy": flags.orderBy, "total": flags.total, "page": flags.page, "pageSize": flags.pageSize })
    this.output(data)
  }
}
