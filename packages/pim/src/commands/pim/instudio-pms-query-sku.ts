...
export default class PimInstudioPmsQuerySku extends MBSCommand {
  static description = '按条件查sku：按条件查sku'
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsQuerySku)

    const data = await this.client.post('/yypms/pms/product/querySku', {}, { params: { "index": flags.index, "sku": flags.sku, "spu": flags.spu, "category_id": flags.categoryId, "tag_id": flags.tagId, "brand_id": flags.brandId, "productname": flags.productname, "createoper": flags.createoper, "createtimestart": flags.createtimestart, "createtimeend": flags.createtimeend, "companyname": flags.companyname, "warehouseid": flags.warehouseid, "positionmname": flags.positionmname, "skustatus": flags.skustatus, "orderby": flags.orderby } })
    this.output(data)
  }
}
