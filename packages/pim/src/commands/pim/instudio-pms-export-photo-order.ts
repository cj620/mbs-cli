...
export default class PimInstudioPmsExportPhotoOrder extends MBSCommand {
  static description = '导出拍照订单：导出拍照订单'
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsExportPhotoOrder)
    ...
    const data = await this.client.post('/yypms/pms/photoOrder/exportPhotoOrder', { "orderType": flags.orderType, "orderId": flags.orderId, "orderStatus": toArray(flags.orderStatus, 'string'), "photoOrderStatus": toArray(flags.photoOrderStatus, 'string'), "developer": toArray(flags.developer, 'string'), "developerLeader": toArray(flags.developerLeader, 'string'), "orderCreateOper": toArray(flags.orderCreateOper, 'string'), "sku": toArray(flags.sku, 'string'), "spu": toArray(flags.spu, 'string'), "signInTimeStart": flags.signInTimeStart, "signInTimeEnd": flags.signInTimeEnd, "returnTimeStart": flags.returnTimeStart, "returnTimeEnd": flags.returnTimeEnd, "currentPage": flags.currentPage, "pageSize": flags.pageSize, "startNum": flags.startNum, "endNum": flags.endNum, "managerNames": flags.managerNames, "lableType": flags.lableType, "createOrderTimeStart": flags.createOrderTimeStart, "createOrderTimeEnd": flags.createOrderTimeEnd, "lossesTimeStart": flags.lossesTimeStart, "lossesTimeEnd": flags.lossesTimeEnd, "signinOper": toArray(flags.signinOper, 'string'), "photoRemark": flags.photoRemark, "spus": toArray(flags.spus, 'string') })
    this.output(data)
  }
}
