...
export default class PimInstudioPmsGetEbaySinglepublishProductCategory extends MBSCommand {
  ...
  static flags = {
    ...
    bestOffer: Flags.integer({ description: '1:true 有议价 else 0' }),
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsGetEbaySinglepublishProductCategory)

    const data = await this.client.post('/yypms/pms/ebaySinglepublishInfoController/getEbaySinglepublishProductCategory', { "id": flags.id, "site": flags.site, "categoryId": flags.categoryId, "categoryName": flags.categoryName, "categoryNameAll": flags.categoryNameAll, "parentCategoryId": flags.parentCategoryId, "categoryLevel": flags.categoryLevel, "leafCategory": flags.leafCategory, "bestOffer": flags.bestOffer, "createby": flags.createby, "createtime": flags.createtime, "variationsEnabled": flags.variationsEnabled, "variationsEnabledCn": flags.variationsEnabledCn })
    this.output(data)
  }
}
