...
export default class PimErpProductGetPurchaseSku2 extends MBSCommand {
  ...
  static flags = {
    sku: Flags.string({ description: 'URL查询参数，当前SKU编号(来源列表行 data-sid)', required: true }),
  }

  async run(): Promise<void> {
    const { flags } = await this.parse(PimErpProductGetPurchaseSku2)

    const data = await this.client.post('/erpProduct/erpProduct/productDetails/getPurchaseSku2', { "sku": flags.sku }, { params: { "sku": flags.sku } })
    this.output(data)
  }
}
