...
export default class PimInstudioPmsFindCategoryNameByCategoryIdEbaySinglepublishInfoController extends MBSCommand {
  static description = '根据分类id查询名字：根据分类id查询名字'
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsFindCategoryNameByCategoryIdEbaySinglepublishInfoController)
    ...
    const data = await this.client.post('/yypms/pms/ebaySinglepublishInfoController/findCategoryNameByCategoryId', { "name": flags.name, "site": flags.site, "vtype": flags.vtype, "spu": flags.spu, "categoryid": flags.categoryid, "id": flags.id, "attributeName": flags.attributeName, "searchParam": flags.searchParam, "title": flags.title, "shopname": flags.shopname, "empName": flags.empName, "picStyle": flags.picStyle, "idList": toArray(flags.idList, 'integer'), "siteList": toArray(flags.siteList, 'object'), "isTranslate": flags.isTranslate })
    this.output(data)
  }
}
