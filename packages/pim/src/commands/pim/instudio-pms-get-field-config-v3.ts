...
export default class PimInstudioPmsGetFieldConfigV3 extends MBSCommand {
  static description = '根据选择的变体属性值, 获取对应的字段配置信息(最新)：根据选择的变体属性值, 获取对应的字段配置信息(最新)'
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsGetFieldConfigV3)
