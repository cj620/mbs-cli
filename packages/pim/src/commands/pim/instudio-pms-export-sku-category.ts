...
export default class PimInstudioPmsExportSkuCategory extends MBSCommand {
  static description = '开发中台 类目维度导出：开发中台 类目维度导出'
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsExportSkuCategory)
    ...
    const data = await this.client.post('/yypms/pms/skuCategory/export', { "times": flags.times, "status": flags.status, "firstCategory": flags.firstCategory, "secondCategory": flags.secondCategory, "categoryList": toArray(flags.categoryList, 'string'), "page": flags.page, "pageSize": flags.pageSize, "startIndex": flags.startIndex, "chartType": flags.chartType, "companyId": flags.companyId, "warningIndexList": toArray(flags.warningIndexList, 'string'), "exportTitleList": toArray(flags.exportTitleList, 'string'), "exportTimeList": toArray(flags.exportTimeList, 'string') })
    this.output(data)
  }
}
