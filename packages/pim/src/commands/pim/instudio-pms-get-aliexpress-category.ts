...
export default class PimInstudioPmsGetAliexpressCategory extends MBSCommand {
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsGetAliexpressCategory)
    ...
    const data = await this.client.post('/yypms/pms/aliexpressSinglepublishController/getAliexpressCategory', { "queryType": flags.queryType, "level": flags.level, "parentCategoryId": flags.parentCategoryId, "keyWord": flags.keyWord, "categoryId": flags.categoryId, "shopName": flags.shopName, "shopNames": toArray(flags.shopNames, 'string'), "description": flags.description, "shopsSplice": flags.shopsSplice, "erpSpu": flags.erpSpu, "erpSku": flags.erpSku, "platformId": flags.platformId, "picStyle": flags.picStyle, "picQueryType": flags.picQueryType, "descriptionPics": toArray(flags.descriptionPics, 'string'), "selectTypes": toArray(flags.selectTypes, 'integer'), "selectType": flags.selectType, "operId": flags.operId, "publishSkuList": toArray(flags.publishSkuList, 'object'), "listId": flags.listId, "listIds": toArray(flags.listIds, 'string'), "findPropertiesBySkus": flags.findPropertiesBySkus, "moneyRate": flags.moneyRate })
    this.output(data)
  }
}
