...
export default class PimInstudioPmsGetEbaySinglepublishShopCategory extends MBSCommand {
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(PimInstudioPmsGetEbaySinglepublishShopCategory)

    const data = await this.client.post('/yypms/pms/ebaySinglepublishInfoController/getEbaySinglepublishShopCategory', { "id": flags.id, "shopname": flags.shopname, "categoryId": flags.categoryId, "categoryName": flags.categoryName, "parentCategoryId": flags.parentCategoryId, "categoryLevel": flags.categoryLevel, "leafCategory": flags.leafCategory, "ordernum": flags.ordernum, "createby": flags.createby, "createtime": flags.createtime })
    this.output(data)
  }
}
