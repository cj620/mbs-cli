...
/**
 * Preserves the published `mbs pim instudio-pms-get-shop-manager-ranking-list` read-only contract from CLI 0.1.58.
 *
 * The command keeps the historical flags and sends requests to `/yypms/pms/middlePanel/getShopManagerRankingList`
 * through the shared authenticated gateway client. It performs no local writes.
 */
export default class PimInstudioPmsGetShopManagerRankingList extends MBSCommand {
    ...
    async run(): Promise<void> {
        const { flags } = await this.parse(PimInstudioPmsGetShopManagerRankingList);
        /**
