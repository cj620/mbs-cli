...
# mbs pim instudio-pms-query-product-picture-by-user-id-product-image

获取当前账号的图片任务：获取当前账号的图片任务

## 用法

```bash
mbs pim instudio-pms-query-product-picture-by-user-id-product-image --id <string> [--spu <string>] [--creater <string>] [--index <string>] [--createrTime <string>] [--projectId <string>] [--state <string>]
```
