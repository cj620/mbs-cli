...
# mbs pim instudio-pms-get-tiktok-local-warehouse-record
...
| `obj.obj.max` | string | 最大（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.addonBefore` | string | AddonBefore（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
