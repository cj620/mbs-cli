# mbs pim instudio-pms-get-main-shop-sites-for-sd
...
| `obj.obj.siteCode` | string | 站点编码（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.shop` | string | 店铺（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.note` | string | 备注（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.siteNameZh` | string | 站点名称ZH（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.englishKeyword` | string | English关键词（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.skus` | string | SKU列表（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
