# mbs pim instudio-pms-get-category-recommend
...
| `obj.obj.childrens` | string | Childrens（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
