# mbs pim instudio-pms-get-all-freight-templates
...
| `obj.obj[].minPrice` | number | 最小价格（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].country` | string | 国家（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].countryCode` | string | 国家编码（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].logisticsValueSmt` | string | 物流值速卖通（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].logisticsValue` | string | 物流值（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].status` | integer | 状态（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].configId` | integer | 配置ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].logisticsChannel` | string | 物流渠道（字段名推断,语义待核实）。前端使用：待核实 | - |
