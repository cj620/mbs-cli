...
| `obj.obj.listId` | string | 列表ID（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.shopName` | string | 店铺名称（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.erpSpu` | string | ERPSPU（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.subject` | string | 科目（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.categoryId` | string | 类目ID（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.categoryList[]` | array | 类目列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.mainPicList[]` | array | 主图片列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.mainPicList[]` | string | - | - |
| `obj.obj.whiteBackGroundPic` | string | WhiteBACKGround图片（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.scenePic` | string | Scene图片（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.videoUrl` | string | 视频URL（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.propertyList[]` | array | 属性列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.propertyDefinitionList[]` | array | 属性Definition列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.qualificationList[]` | array | Qualification列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.productType` | integer | 商品类型（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.packageType` | integer | 包裹类型（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.productUnit` | integer | 商品单位（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.lotNum` | integer | LOT数量（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.skuInfoList[]` | array | SKU信息列表（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.webDescription` | string | 网页描述（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.publishStatus` | integer | 刊登状态（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.imageRelate` | string | 图片Relate（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.publishResult` | string | 刊登结果（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.reviseInventory` | integer | 修改库存（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj.hsCodeJson` | string | HS编码JSON（字段名推断,语义待核实）。前端使用：否 | - |
