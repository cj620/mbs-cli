...
# mbs pim instudio-pms-query-product-picture-by-user-id3

获取所有的图片任务-新：获取所有的图片任务-新

## 用法

```bash
mbs pim instudio-pms-query-product-picture-by-user-id3 --id <string> [--spu <string>] [--creater <string>] [--index <string>] [--createrTime <string>] [--state <string>] [--oper <string>] [--flag <string>] [--createByPicture <string>] [--developType <string>] [--isTwoPicture <string>] [--createDateStart <string>] [--createDateEnd <string>] [--searchCompanyId <integer>]
```
...
| `oper` | oper | query | string | 否 | - | 操作（字段名推断,语义待核实） |
| `flag` | flag | query | string | 否 | - | 标志（字段名推断,语义待核实） |
| `createByPicture` | createByPicture | query | string | 否 | - | 创建人图片（字段名推断,语义待核实） |
| `developType` | developType | query | string | 否 | - | Develop类型（字段名推断,语义待核实） |
| `isTwoPicture` | isTwoPicture | query | string | 否 | - | 是否两个图片（字段名推断,语义待核实） |
| `createDateStart` | createDateStart | query | string | 否 | - | 创建日期开始（字段名推断,语义待核实） |
| `createDateEnd` | createDateEnd | query | string | 否 | - | 创建日期结束（字段名推断,语义待核实） |
| `searchCompanyId` | searchCompanyId | query | integer | 否 | - | 搜索公司ID（字段名推断,语义待核实） |
