...
# mbs pim instudio-pms-find-category-name-by-category-id-ozon-singlepublish-info-controller
...
| `obj.obj.categoryNameCN` | string | 类目名称中文（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.categoryName` | string | 类目名称（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.leafCategory` | string | LEAF类目（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
