...
| `obj.obj[].shipTo` | string | 发货（字段名推断,语义待核实）。前端使用：待核实 | - |
...
| `obj.obj[].createOper` | integer | 创建操作（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].updateOper` | integer | 更新操作（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].memo` | string | 备注（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].num` | integer | 数量（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].type` | integer | 1 相对加减 2相对百分数。前端使用：待核实 | - |
| `obj.obj[].shipToMap[]` | array | 发货MAP（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].emps[]` | array | EMPS（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].emps[]` | string | - | - |
| `obj.obj[].checkTimeStart` | string | 校验时间开始（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].checkTimeEnd` | string | 校验时间结束（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].itemId` | string | 条目ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].shopName` | string | 店铺名称（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].jumpUrl` | string | JUMPURL（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].erpSpu` | string | ERPSPU（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].recordId` | integer | 记录ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].configId` | integer | 配置ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].title` | string | 标题（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].publishSpu` | string | 刊登SPU（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].publishDate` | string | 刊登日期（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].ids[]` | array | ID列表（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].ids[]` | integer | - | - |
| `obj.obj[].startIndex` | integer | 开始索引（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].pageSize` | integer | 每页条数（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].currentPage` | integer | 当前页码（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].totalPage` | integer | 总数页码（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].totalSize` | integer | 总大小（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].itemSubmits[]` | array | 条目Submits（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].itemSubmits[]` | string | - | - |
| `obj.obj[].itemSubmitStr` | string | 条目提交字符串（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].submitBy` | string | 提交人（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].shiptotype` | string | Shiptotype（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].submitDate` | string | 提交日期（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].submitType` | integer | 提交类型（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].batchId` | string | 批次ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].status` | integer | 状态（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].response` | string | 响应（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].refreshDate` | string | 刷新日期（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].freightId` | string | 运费ID（字段名推断,语义待核实）。前端使用：待核实 | - |
