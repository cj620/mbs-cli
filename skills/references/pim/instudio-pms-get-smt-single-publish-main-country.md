...
# mbs pim instudio-pms-get-smt-single-publish-main-country
...
| `obj.obj[].key` | string | 键（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].values[]` | array | 值列表（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].other` | string | 其他（字段名推断,语义待核实）。前端使用：待核实 | - |
