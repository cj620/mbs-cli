# mbs pim instudio-pms-get-shopee-voucher-template-by-id
...
| `obj.obj.code` | string | 编码（字段名推断,语义待核实）。前端使用：待核实 | - |
...
| `obj.obj.voucher` | object | 凭证（字段名推断,语义待核实）。前端使用：待核实 | - |
...
| `obj.obj.display_channel` | integer | 1 所有页面 2 优惠券分享 3 特定渠道。前端使用：待核实 | - |
