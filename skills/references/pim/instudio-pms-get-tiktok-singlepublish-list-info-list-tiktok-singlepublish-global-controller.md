# mbs pim instudio-pms-get-tiktok-singlepublish-list-info-list-tiktok-singlepublish-global-controller
...
| `obj.obj.siteShops` | string | 站点店铺列表（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.publishShopInfoMapList` | string | 刊登店铺信息MAP列表（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.publishAwaitList` | string | 刊登Await列表（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
