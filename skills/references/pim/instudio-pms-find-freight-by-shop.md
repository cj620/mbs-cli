# mbs pim instudio-pms-find-freight-by-shop
...
| `obj.obj[].freightTemplateNameEn` | string | 运费模板名称英文（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].freightTemplateNameZh` | string | 运费模板名称ZH（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].minPrice` | string | 最小价格（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].maxPrice` | number | 最大价格（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].properties` | string | 属性（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].shopName` | string | 店铺名称（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].freightTemplateId` | string | 运费模板ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].freightTemplateName` | string | 运费模板名称（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].configurationId` | integer | 配置ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].createBy` | string | 创建人（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].groupName` | string | 分组名称（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].firstWeightReduction` | number | 首个重量Reduction（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].standardPriceNew` | number | Standard价格新（字段名推断,语义待核实）。前端使用：待核实 | - |
