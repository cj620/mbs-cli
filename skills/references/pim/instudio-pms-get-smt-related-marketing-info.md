...
# mbs pim instudio-pms-get-smt-related-marketing-info

查询速卖通Related营销信息：查询速卖通Related营销信息(源码无注释,按方法名推断)

## 用法

```bash
mbs pim instudio-pms-get-smt-related-marketing-info [--id <integer>] [--templateName <string>] [--templateInfo <string>] [--createBy <string>] [--updateBy <string>] [--url <string>]
```
...
| `obj.obj.templateName` | string | 模板名称（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
