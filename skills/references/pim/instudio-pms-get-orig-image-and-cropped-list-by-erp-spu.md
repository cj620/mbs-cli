...
# mbs pim instudio-pms-get-orig-image-and-cropped-list-by-erp-spu

通过spu获取sku图片的原图和裁剪后的图片：通过spu获取sku图片的原图和裁剪后的图片

## 用法

```bash
mbs pim instudio-pms-get-orig-image-and-cropped-list-by-erp-spu --erpSpu <string>
```
...
| `obj.obj[].base64Img` | string | 基础64图片（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj[].croImg` | string | 裁剪后的图。前端使用：是（列表行字段，行号待核实） | - |
| `obj.obj[].marginLeft` | integer | MarginLEFT（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj[].marginRight` | integer | MarginRight（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj[].marginTop` | integer | Margin排行（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj[].marginBottom` | integer | MarginBottom（字段名推断,语义待核实）。前端使用：否 | - |
| `obj.obj[].extra` | object | 给前端用的，后端没有用。。前端使用：是（列表行字段，行号待核实） | - |
