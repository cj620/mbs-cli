...
| `obj.obj[].propertyId` | string | 属性ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].propertyNameEn` | string | 属性名称英文（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].propertyNameZh` | string | 属性名称ZH（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].showType` | string | 展示类型（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].isRequired` | integer | 是否必填（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].isCustomizedName` | integer | 是否Customized名称（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].isCustomizedPic` | integer | 是否Customized图片（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].propValueList[]` | array | PROP值列表（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].propValue` | string | PROP值（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].ifSkuProp` | integer | IFSKUPROP（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].supportEnumInput` | integer | SupportENUMInput（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].inputType` | string | Input类型（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].units[]` | array | 单位（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].units[]` | string | - | - |
| `obj.obj[].subId` | string | 子ID（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].subId1` | string | 子ID1（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].subId2` | string | 子ID2（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].subId3` | string | 子ID3（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].addSubStr` | string | 新增子字符串（字段名推断,语义待核实）。前端使用：待核实 | - |
| `obj.obj[].materialRatio` | integer | Material比例（字段名推断,语义待核实）。前端使用：待核实 | - |
