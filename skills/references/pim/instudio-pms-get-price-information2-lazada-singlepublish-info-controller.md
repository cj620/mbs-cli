...
# mbs pim instudio-pms-get-price-information2-lazada-singlepublish-info-controller
...
```bash
mbs pim instudio-pms-get-price-information2-lazada-singlepublish-info-controller --list <array<object>>
```
...
| `obj.obj._priceError` | string | 价格错误（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.price` | string | 价格（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.currency` | string | 币种（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.originalPrice` | string | 原始价格（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.salePrice` | string | 销售价格（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
...
| `obj.obj.images` | string | 图片列表（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
