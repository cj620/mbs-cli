...
# mbs pim instudio-pms-ozon-image-cropped-list
...
```bash
mbs pim instudio-pms-ozon-image-cropped-list [--erpSpu <string>] [--listingId <integer>] [--croppedList <array<object>>] [--spuFormList <array<object>>]
```
...
| `listingId` | listingId | body | integer | 否 | - | 当前编辑的 id |
| `croppedList` | croppedList | body | array<object> | 否 | - | 当前需要裁剪的数据 |
| `spuFormList` | spuFormList | body | array<object> | 否 | - | spu的原图数据 |
...
| `obj.obj.listingId` | integer | 当前编辑的 id。前端使用：否 | - |
| `obj.obj.croppedList[]` | array | 当前需要裁剪的数据。前端使用：是（取值，行号待核实） | - |
| `obj.obj.spuFormList[]` | array | spu的原图数据。前端使用：否 | - |
...
| `obj.obj.imgGenerateStatus` | string | 图片生成状态（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.pageImageUrl` | string | 页码图片URL（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
| `obj.obj.el` | string | EL（前端推断字段）。前端使用：是（前端推断，行号待核实） | - |
