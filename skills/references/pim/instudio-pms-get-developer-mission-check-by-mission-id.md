# mbs pim instudio-pms-get-developer-mission-check-by-mission-id
...
```bash
mbs pim instudio-pms-get-developer-mission-check-by-mission-id [--missionId <string>] [--checkBy <string>] [--state <string>] [--id <string>] [--num <string>] [--content <string>] [--missionList <array<string>>] [--updateTime <string>] [--skuPropertiesInfos <array<object>>] [--skuProperties <array<object>>] [--reason <string>]
```
...
| `checkBy` | checkBy | body | string | 否 | - | 校验人（字段名推断,语义待核实） |
| `state` | state | body | string | 否 | - | 状态（字段名推断,语义待核实） |
| `id` | id | body | string | 否 | - | ID（字段名推断,语义待核实） |
| `num` | num | body | string | 否 | - | 数量（字段名推断,语义待核实） |
| `content` | content | body | string | 否 | - | 内容（字段名推断,语义待核实） |
| `missionList` | missionList | body | array<string> | 否 | - | Mission列表（字段名推断,语义待核实） |
| `updateTime` | updateTime | body | string | 否 | - | 更新时间（字段名推断,语义待核实） |
| `skuPropertiesInfos` | skuPropertiesInfos | body | array<object> | 否 | - | SKU属性信息（字段名推断,语义待核实） |
| `skuProperties` | skuProperties | body | array<object> | 否 | - | SKU属性（字段名推断,语义待核实） |
